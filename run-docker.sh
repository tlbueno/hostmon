...
if [[ $(docker network ls -q -f 'Name=${MONITORING_NETWORK_NAME}') -eq 0 ]] ; then
...
if [[ ! -e "prometheus/prometheus.yml" ]]; then
    ...
    docker create --name prometheus_dummy prom/prometheus
    echo "copy default prometheus config file"
    docker cp prometheus_dummy:/etc/prometheus/prometheus.yml prometheus/
    echo "removing dummy prometheus container"
    docker rm prometheus_dummy
fi

echo "starting prometheus container"
docker run --rm --detach --name prometheus --publish 9090:9090 --network ${MONITORING_NETWORK_NAME} --volume "${PWD}/prometheus/prometheus.yml:/etc/prometheus/prometheus.yml:Z" prom/prometheus
echo ""
...
if [[ ! -d "grafana/etc" ]]; then
    ...
    docker create --name grafana_dummy grafana/grafana
    echo "copy default grafana config directory"
    docker cp grafana_dummy:/etc/grafana/. grafana/etc
    echo "removing dummy grafana container"
    docker rm grafana_dummy
fi
...
if [[ ! -d "grafana/data" ]]; then
    ...
    docker cp grafana_dummy:/var/lib/grafana/. grafana/data
    echo "removing dummy grafana container"
    docker rm grafana_dummy
fi

echo "starting grafana container"
docker run --rm --detach --name grafana --publish 3000:3000 --user root:root --network ${MONITORING_NETWORK_NAME} --volume "${PWD}/grafana/etc:/etc/grafana:Z" --volume "${PWD}/grafana/data:/var/lib/grafana:Z" grafana/grafana
echo ""
